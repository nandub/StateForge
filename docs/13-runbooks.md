...
6. Review `lock-contention` as expected fenced-write contention, and block only if it pairs with
   data-verification errors or unreadable records.
7. If cleanup runs during active writes, verify it skipped transient file-read races instead of
   quarantining or deleting currently written records.
8. Archive the reviewed reports with the release evidence.
